...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($car_names as $i => $cn) {
        if ($cn !== '') {
            if (strlen($cn) > 150) $errors['car_name_'.$i] = 'Maximum 150 caractères.';
            $cars[] = [
                'name'        => $cn,
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            foreach ($avantages as $a)     $stmtAI->execute(['avantage',    $a, $outilId]);
            foreach ($inconvenients as $i) $stmtAI->execute(['inconvenient',$i, $outilId]);
            ...
            <div class="ao-row-2">
                <div class="ao-field">
                    ...
                    <input type="text" name="nom" value="<?= htmlspecialchars($old['nom'] ?? '') ?>"
                        ...
                        oninput="clearErr(this)" autocomplete="off">
        ...
        <div class="ao-card ao-card-hidden" id="step2">
            ...
            <div class="ao-ai-grid">
                ...
                <div class="ao-ai-col ao-ai-col--pro">
                        ...
                        foreach ($avOld as $idx => $av): ?>
                            <div class="ao-ai-row">
                                <input type="text" name="avantages[]"
                                    value="<?= htmlspecialchars($av) ?>"
                                    placeholder="Ex : Gratuit, Rapide, API disponible…"
                                    class="ao-input">
                    ...
                    <button type="button" class="ao-ai-add ao-ai-add--pro" onclick="addRow('avantagesList','avantages[]','Ex : Gratuit, Rapide…')">
                ...
                <div class="ao-ai-col ao-ai-col--con">
                    ...
                    <div id="inconvenientsList">
                        ...
                        foreach ($inOld as $idx => $inc): ?>
                            <div class="ao-ai-row">
                                <input type="text" name="inconvenients[]"
                                    value="<?= htmlspecialchars($inc) ?>"
                                    placeholder="Ex : Payant, Lent, Peu précis…"
                                    class="ao-input">
                                <button type="button" class="ao-ai-remove" onclick="removeRow(this)" title="Supprimer">
                                    <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                        <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                    ...
                    <button type="button" class="ao-ai-add ao-ai-add--con" onclick="addRow('inconvenientsList','inconvenients[]','Ex : Payant, Lent…')">
                        <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                    ...
                    <div class="ao-perf-field">
                        ...
                        <div class="ao-perf-input-wrap">
                            <input type="number" name="perf_<?= $key ?>"
                                value="<?= htmlspecialchars($old['perf_'.$key] ?? '') ?>"
                            ...
                            <div class="ao-perf-bar-wrap">
                                <div class="ao-perf-bar" id="bar_<?= $key ?>"
                                    style="width:<?= isset($old['perf_'.$key]) && is_numeric($old['perf_'.$key]) ? ($old['perf_'.$key]*10).'%' : '0%' ?>;background:<?= $meta['color'] ?>"></div>
                            </div>
                        </div>
                            ...
                            <div class="ao-car-fields">
                                ...
                                <div class="ao-field" style="margin-bottom:0">
                                    ...
                                    <input type="text" name="car_desc[]"
                                        ...
                                        placeholder="Brève description de la caractéristique"
                                        class="ao-input">
                                </div>
...
.ao-back:hover { color: var(--primary); }
.ao-current { font-weight: 800; color: var(--navy); }

/* ── Hero ─────────────────────────────────────── */
.ao-hero { text-align: center; margin-bottom: 28px; }
.ao-title { font-size: 32px; font-weight: 900; letter-spacing: -1px; color: var(--navy); margin-bottom: 8px; }
...
.ao-step {
    ...
    color: #94A3B8; transition: .2s; white-space: nowrap;
}
.ao-step.active { color: #1B2A4A; background: rgba(245,184,0,.15); }
.ao-step.done { color: #16a34a; }
.ao-step-num {
...
.ao-step.active .ao-step-num { background: #F5B800; color: #fff; }
.ao-step.done .ao-step-num { background: #16a34a; color: #fff; }
.ao-step-line { flex: 1; height: 2px; background: #E2E8F0; min-width: 20px; }
...
.ao-card-icon--green { background: linear-gradient(135deg,#16a34a,#4ade80); box-shadow: 0 8px 20px rgba(22,163,74,.2); }
.ao-card-icon--blue  { background: linear-gradient(135deg,#2563eb,#60a5fa); box-shadow: 0 8px 20px rgba(37,99,235,.2); }
.ao-card-icon--purple{ background: linear-gradient(135deg,#7c3aed,#a78bfa); box-shadow: 0 8px 20px rgba(124,58,237,.2); }

.ao-card-title { font-size: 18px; font-weight: 800; color: #1B2A4A; margin: 0 0 4px; }
.ao-card-sub   { font-size: 13px; color: #94A3B8; margin: 0; }

/* ── Fields ───────────────────────────────────── */
.ao-row-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
.ao-field { display: flex; flex-direction: column; margin-bottom: 22px; }
.ao-field-sm { max-width: 220px; }
.ao-field-xs { max-width: 140px; }
...
.ao-charcount { margin-left: auto; font-size: 11px; color: var(--muted); text-transform: none; }
.ao-req { color: #EF4444; }

.ao-input {
    width: 100%; padding: 13px 15px; border-radius: 14px;
    border: 1.5px solid #1B2A4A; background: #fff;
    font-size: 14px; transition: .2s; box-sizing: border-box;
}
.ao-input:hover { border-color: #D1D5DB; }
.ao-input:focus { outline: none; border-color: #F5B800; box-shadow: 0 0 0 4px rgba(245,184,0,.15); }
.ao-input::placeholder { color: #B8C1CC; }
.ao-input-err { border-color: #EF4444 !important; background: #FFF6F6 !important; }
.ao-input-err:focus { box-shadow: 0 0 0 4px rgba(239,68,68,.12); }
.ao-textarea { resize: vertical; min-height: 130px; line-height: 1.7; }
.ao-select-wrap { position: relative; }
.ao-select { appearance: none; padding-right: 38px; }
.ao-select-icon { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); color: #94A3B8; pointer-events: none; }
.ao-input-icon-wrap { position: relative; }
.ao-input-icon { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: #94A3B8; }
.ao-input-has-icon { padding-left: 40px; }
...
/* ── Error ────────────────────────────────────── */
.ao-err { margin-top: 6px; color: #EF4444; font-size: 12px; font-weight: 600; }
...
.ao-btn-cancel, .ao-btn-prev {
    padding: 12px 18px; border-radius: 12px;
    border: 1px solid #E5E7EB; text-decoration: none;
    color: #64748B; font-weight: 700; transition: .2s;
    display: flex; align-items: center; gap: 6px;
    background: transparent; cursor: pointer; font-size: 14px;
}
.ao-btn-cancel:hover, .ao-btn-prev:hover { border-color: #CBD5E1; background: #c4c7ca; }
.ao-btn-next {
...
.ao-btn-next:hover { transform: translateY(-2px); box-shadow: 0 15px 35px rgba(15,23,42,.30); }
.ao-btn-submit {
    padding: 13px 28px; border: none; border-radius: 14px;
    cursor: pointer; font-weight: 800; color: #fff;
    display: flex; align-items: center; gap: 8px;
    background: linear-gradient(135deg,#1B2A4A,#1B2A4A);
    box-shadow: 0 10px 30px rgba(68,98,168,.25); transition: .2s; font-size: 14px;
}
.ao-btn-submit:hover { transform: translateY(-2px); box-shadow: 0 15px 35px rgba(15,23,42,.30); }
.ao-btn-submit:disabled { opacity: .7; cursor: not-allowed; }

/* ── Avantages / Inconvénients ────────────────── */
.ao-ai-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; }
.ao-ai-col { display: flex; flex-direction: column; gap: 10px; }
.ao-ai-col--pro { }
.ao-ai-col--con { }
.ao-ai-col-header {
    display: flex; align-items: center; gap: 8px;
...
.ao-ai-row .ao-input { flex: 1; margin-bottom: 0; padding: 10px 14px; }
.ao-ai-remove {
...
.ao-ai-remove:hover { background: #FEF2F2; border-color: #FECACA; color: #DC2626; }
.ao-ai-add {
    margin-top: 4px; padding: 9px 14px; border-radius: 10px;
    border: 1.5px dashed; font-size: 12px; font-weight: 700;
    display: flex; align-items: center; gap: 6px;
    cursor: pointer; background: transparent; transition: .2s; align-self: flex-start;
}
.ao-ai-add--pro { border-color: #86efac; color: #16a34a; }
.ao-ai-add--pro:hover { background: #f0fdf4; }
.ao-ai-add--con { border-color: #fca5a5; color: #dc2626; }
.ao-ai-add--con:hover { background: #fef2f2; }
...
.ao-perf-field { display: flex; flex-direction: column; gap: 8px; }
.ao-perf-icon { font-size: 16px; }
.ao-perf-input-wrap { display: flex; flex-direction: column; gap: 6px; }
.ao-perf-input {
    text-align: center; font-size: 20px; font-weight: 800;
    padding: 14px 10px !important;
}
.ao-perf-input:focus { border-color: var(--perf-accent) !important; box-shadow: 0 0 0 4px color-mix(in srgb, var(--perf-accent) 20%, transparent) !important; }
.ao-perf-bar-wrap {
...
.ao-car-row-inner { display: flex; align-items: flex-start; gap: 12px; }
.ao-car-fields { flex: 1; display: grid; grid-template-columns: 1fr 1fr auto; gap: 14px; }
.ao-car-remove {
    flex-shrink: 0; width: 34px; height: 34px; margin-top: 24px;
    border-radius: 10px; border: 1px solid #E5E7EB;
    background: #fff; color: #94A3B8;
    display: flex; align-items: center; justify-content: center;
    cursor: pointer; transition: .2s;
}
.ao-car-remove:hover { background: #FEF2F2; border-color: #FECACA; color: #DC2626; }
.ao-car-add {
...
.ao-car-add:hover { border-color: #7c3aed; color: #7c3aed; background: rgba(124,58,237,.04); }
...
@media (max-width: 768px) {
    .ao-row-2, .ao-ai-grid { grid-template-columns: 1fr; }
    .ao-car-fields { grid-template-columns: 1fr; }
    .ao-card { padding: 22px; }
    .ao-step-actions { flex-direction: column; gap: 12px; }
    .ao-btn-cancel, .ao-btn-prev, .ao-btn-next, .ao-btn-submit { width: 100%; justify-content: center; }
    .ao-field-sm, .ao-field-xs { max-width: 100%; }
    .ao-steps { flex-wrap: wrap; gap: 4px; }
    .ao-step-line { display: none; }
    .ao-step-label { display: none; }
    .ao-perf-grid { grid-template-columns: repeat(2, 1fr); }
}
</style>
