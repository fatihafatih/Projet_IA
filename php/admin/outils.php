...
if(in_array($action,['valider','refuser','supprimer','activer'])&&$id>0){
    $o=$pdo->prepare("SELECT * FROM outils_ia WHERE ID_OUTILS_IA=?"); $o->execute([$id]); $o=$o->fetch();
    if($o) switch($action){
        case 'valider':
            ...
            $msg='✅ Outil validé — notification envoyée au créateur.'; break;
        case 'refuser':
            ...
            $msg='⛔ Outil refusé — notification envoyée au créateur.'; $msgType='warning'; break;
        case 'activer':
            $pdo->prepare("UPDATE outils_ia SET status='actif',ID_USERS_VALID=? WHERE ID_OUTILS_IA=?")->execute([$_SESSION['user_id'],$id]);
            $msg='✅ Outil réactivé.'; break;
        case 'supprimer':
            $pdo->prepare("DELETE FROM outils_ia WHERE ID_OUTILS_IA=?")->execute([$id]);
            $msg='🗑️ Outil supprimé.'; $msgType='danger'; break;
    }
    header('Location: outils.php?filter='.($_GET['filter']??'all').'&msg='.urlencode($msg).'&type='.$msgType); exit;
}
    ...
    if(!$nom){$msg='Nom obligatoire.';$msgType='danger';}
        ...
        if($pid>0){
            ...
            foreach($modelIds as $mid){
                if($mid>0) $pdo->prepare("INSERT IGNORE INTO tool_models(ID_OUTILS_IA,ID_MODEL) VALUES(?,?)")->execute([$pid,$mid]);
            }
            ...
            foreach($modelIds as $mid){
                if($mid>0) $pdo->prepare("INSERT IGNORE INTO tool_models(ID_OUTILS_IA,ID_MODEL) VALUES(?,?)")->execute([$newId,$mid]);
            }
            $msg='✅ Outil ajouté et publié.';
        }
        header('Location: outils.php?msg='.urlencode($msg)); exit;
    }
}

/* ── Liste / filtres / pagination ── */
$filter=$_GET['filter']??'all'; $q=trim($_GET['q']??''); $page=max(1,(int)($_GET['page']??1)); $pp=12;
$where='WHERE 1=1'; $params=[];
if($filter==='en_attente'){$where.=" AND o.status='en_attente'";}
elseif($filter==='actif'){$where.=" AND o.status='actif'";}
elseif($filter==='inactif'){$where.=" AND o.status='inactif'";}
if($q){$where.=" AND (o.nom LIKE ? OR o.description LIKE ?)";$params[]="%$q%";$params[]="%$q%";}

$total=$pdo->prepare("SELECT COUNT(*) FROM outils_ia o $where"); $total->execute($params); $total=$total->fetchColumn();
...
$outils->execute($params); $outils=$outils->fetchAll();
...
foreach($tmRows as $r) $modelsByTool[$r['ID_OUTILS_IA']][]=$r['name'];
...
foreach($tmIds as $r) $toolModelIds[$r['ID_OUTILS_IA']][]=(int)$r['ID_MODEL'];
...
<style>
.t-thumb{width:38px;height:38px;object-fit:contain;border-radius:8px;border:1px solid var(--adm-border,#e2e8f0);background:#f8fafc;padding:3px;flex-shrink:0}
.tool-cell{display:flex;align-items:center;gap:10px}
.model-badges{display:flex;flex-wrap:wrap;gap:4px}
.badge-model{font-size:.65rem;font-weight:600;padding:2px 7px;border-radius:20px;background:var(--adm-badge-bg,#ede9fe);color:var(--adm-badge-text,#5b21b6);white-space:nowrap}
.img-preview-wrap{display:flex;align-items:center;gap:12px;margin-top:6px}
.img-preview{width:56px;height:56px;object-fit:contain;border:1px solid var(--adm-border,#e2e8f0);border-radius:10px;background:#f8fafc;padding:4px;display:none}
.img-preview-placeholder{width:56px;height:56px;border:1.5px dashed var(--adm-border,#cbd5e1);border-radius:10px;display:flex;align-items:center;justify-content:center;color:var(--adm-muted,#94a3b8);font-size:.68rem;text-align:center;line-height:1.3;padding:4px}
.models-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px;max-height:220px;overflow-y:auto;padding:4px 2px}
.model-cb-label{display:flex;align-items:center;gap:8px;padding:7px 10px;border:1px solid var(--adm-border,#e2e8f0);border-radius:8px;cursor:pointer;font-size:.82rem;transition:background .15s,border-color .15s;line-height:1.3}
.model-cb-label:hover{background:var(--adm-hover,#f1f5f9);border-color:var(--adm-blue,#3b82f6)}
.model-cb-label input[type="checkbox"]{accent-color:var(--adm-blue,#3b82f6);width:15px;height:15px;flex-shrink:0}
.model-provider-tag{font-size:.68rem;color:var(--adm-muted,#64748b);display:block}
...
<div class="adm-main">
  ...
  <div class="stabs">
    ...
    <a href="outils.php?filter=<?=$k?>" class="stab <?=$filter===$k?'active':''?>"><?=$l?> <span class="stab-n"><?=$counts[$k]?></span></a>
    <?php endforeach; ?>
  </div>
  ...
  <div class="adm-section">
    <div class="adm-sec-head"><h2><?=$total?> outil<?=$total>1?'s':''?></h2></div>
    <div class="tbl-wrap"><table class="adm-table">
      <thead>
        <tr><th>#</th><th>Outil</th><th>Catégorie</th><th>Modèles</th><th>Statut</th><th>Créé par</th><th>Actions</th></tr>
      </thead>
      <tbody>
      <?php if(empty($outils)): ?>
        <tr><td colspan="7" style="text-align:center;padding:40px;color:var(--adm-muted)">Aucun outil.</td></tr>
      <?php endif; ?>
      ...
      <tr>
        ...
        <td>
          <div class="tool-cell">
            <?php if(!empty($o['logo_url'])): ?>
              <img src="<?=htmlspecialchars($o['logo_url'])?>" class="t-thumb" alt="" onerror="this.style.display='none'">
            <?php endif; ?>
            <div>
              <div style="font-weight:600;font-size:.875rem"><?=htmlspecialchars($o['nom'])?></div>
              <?php if($o['url']): ?>
                <a href="<?=htmlspecialchars($o['url'])?>" target="_blank" style="font-size:.72rem;color:var(--adm-blue);text-decoration:none">↗ Site</a>
              <?php endif; ?>
            </div>
          </div>
        </td>
        ...
        <td>
          <div class="model-badges">
            ...
            <?php endforeach; ?>
            <?php if(empty($modelsByTool[$o['ID_OUTILS_IA']])): ?><span style="font-size:.75rem;color:var(--adm-muted)">—</span><?php endif; ?>
          </div>
        </td>
        <td><span class="st-pill <?=$o['status']?>"><?=$o['status']==='en_attente'?'En attente':ucfirst($o['status'])?></span></td>
        <td style="font-size:.8rem;color:var(--adm-muted)"><?=htmlspecialchars($o['cr_nom']??'—')?></td>
        <td>
          <div class="act-btns">
            <?php if($o['status']==='en_attente'): ?>
              <a href="outils.php?action=valider&id=<?=$o['ID_OUTILS_IA']?>&filter=<?=$filter?>" class="btn-val" onclick="return confirm('Valider et notifier ?')">✓ Valider</a>
              <button type="button" class="btn-ref" onclick="openRefusalModal(<?=$o['ID_OUTILS_IA']?>)">✗ Refuser</button>
            <?php elseif($o['status']==='actif'): ?>
              <a href="outils.php?action=refuser&id=<?=$o['ID_OUTILS_IA']?>&filter=<?=$filter?>" class="btn-ref" title="Désactiver" onclick="return confirm('Désactiver ?')">⊘</a>
            <?php else: ?>
              <a href="outils.php?action=activer&id=<?=$o['ID_OUTILS_IA']?>&filter=<?=$filter?>" class="btn-val" onclick="return confirm('Réactiver ?')">↺</a>
            <?php endif; ?>
            <button class="btn-edt" data-id="<?=$o['ID_OUTILS_IA']?>" onclick="openEditById(this.dataset.id)">✎</button>
            <a href="outils.php?action=supprimer&id=<?=$o['ID_OUTILS_IA']?>&filter=<?=$filter?>" class="btn-del" onclick="return confirm('Supprimer définitivement ?')">🗑</a>
          </div>
        </td>
      </tr>
      <?php endforeach; ?>
    ...
    <div class="adm-pag">
      <?php for($p=1;$p<=$pages;$p++): ?>
        <a href="?filter=<?=$filter?>&q=<?=urlencode($q)?>&page=<?=$p?>" class="pag-btn <?=$p===$page?'active':''?>"><?=$p?></a>
      <?php endfor; ?>
    </div>
    <?php endif; ?>
  </div>
  ...
  <div class="adm-modal">
    ...
    <form method="POST" class="adm-modal-body">
      ...
      <div class="form-group">
        ...
        <textarea class="form-control" name="description" rows="3" placeholder="Décrivez brièvement l'outil…"></textarea>
      </div>
      <div class="form-group">
        <label class="form-label">URL du logo</label>
        <input class="form-control" type="url" id="a_logo_url" name="logo_url" placeholder="https://exemple.com/logo.png" oninput="previewImage(this,'a_preview','a_ph')">
        <div class="img-preview-wrap">
          <img id="a_preview" class="img-preview" alt="" onerror="this.style.display='none';document.getElementById('a_ph').style.display='flex'">
        ...
        <div class="models-grid">
          <?php foreach($allModels as $m): ?>
            <label class="model-cb-label">
              ...
              <span><?=htmlspecialchars($m['name'])?><span class="model-provider-tag"><?=htmlspecialchars($m['provider_name']??'—')?></span></span>
            </label>
          <?php endforeach; ?>
        </div>
      </div>
...
<div class="adm-modal-overlay" id="editModal" onclick="if(event.target===this)closeModal('editModal')">
  <div class="adm-modal">
    ...
    <form method="POST" class="adm-modal-body">
      ...
      <div class="form-group">
        <label class="form-label">URL du logo</label>
        <input class="form-control" type="url" id="e_logo_url" name="logo_url" oninput="previewImage(this,'e_preview','e_ph')">
        <div class="img-preview-wrap">
          <img id="e_preview" class="img-preview" alt="" onerror="this.style.display='none';document.getElementById('e_ph').style.display='flex'">
  ...
  <div class="adm-modal">
    ...
    <form method="POST" id="refuseForm" class="adm-modal-body" onsubmit="return validateRefusalForm()">
      ...
      <div class="form-group">
        ...
        <textarea id="refusalCauseInput" name="refusal_cause" class="form-control" rows="4" placeholder="Expliquez pourquoi l'outil est refusé."></textarea>
